...
// brought this in because it's not been merged to DefinitelyTyped

type Color = string;
type DygraphPoint = Array<Date | number | number[]>;
type DygraphCoords = [number, number];
type DygraphRange = [number, number];
type DygraphData = Function | string | Array<DygraphPoint>;
...
interface Dygraph {
  new (div: HTMLElement | string, file: DygraphData, attrs: DygraphOptions, opt_fourth_param?: any): Dygraph;
  adjustRoll(length: number): void;
  annotations(): any;
  clearSelection(): void;
  destroy(): void;
  eventToDomCoords(event: any): void;
  getArea(): Area;
  ...
  rollPeriod(): number;
  setAnnotations(ann: Array<any>, suppressDraw?: boolean): void;
  setSelection(row: number, series?: string, locked?: boolean): void;
  ...
  toString(): string;
  updateOptions(attrs: DygraphOptions, block_redraw?: boolean): void;
  visibility(): boolean;
